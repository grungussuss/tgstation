...
/datum/element/footstep/proc/prepare_step(mob/living/source)
	...
	if(source.body_position == LYING_DOWN) //play crawling sound if we're lying
		if(turf.footstep)
			playsound(turf, 'sound/effects/footstep/crawl1.ogg', 15 * volume, falloff_distance = 1, vary = sound_vary, pressure_affected = TRUE, ignore_walls = FALSE)
		return
...
/datum/element/footstep/proc/play_simplestep(mob/living/source, atom/oldloc, direction, forced, list/old_locs, momentum_change)
	...
	if(isfile(footstep_sounds) || istext(footstep_sounds))
		playsound(source.loc, footstep_sounds, volume, falloff_distance = 1, vary = sound_vary, pressure_affected = TRUE, ignore_walls = FALSE)
		return

	var/turf_footstep = prepared_steps[footstep_type]
	if(isnull(turf_footstep) || !footstep_sounds[turf_footstep])
		return
	playsound(source.loc, pick(footstep_sounds[turf_footstep][1]), footstep_sounds[turf_footstep][2] * volume, TRUE, footstep_sounds[turf_footstep][3] + e_range, falloff_distance = 1, vary = sound_vary, pressure_affected = TRUE, ignore_walls = FALSE)
	...
	if((source.wear_suit?.body_parts_covered | source.w_uniform?.body_parts_covered | source.shoes?.body_parts_covered) & FEET)
		...
		if(!isnull(shoestep_type) && footstep_sounds[shoestep_type]) // shoestep type can be null
			heard_clients = playsound(source.loc, pick(footstep_sounds[shoestep_type][1]),
				...
				TRUE,
				footstep_sounds[shoestep_type][3] + e_range + range_adjustment, falloff_distance = 1, vary = sound_vary, ignore_walls = FALSE)
	else
		// we are barefoot

		if(source.dna.species.special_step_sounds)
			heard_clients = playsound(source.loc, pick(source.dna.species.special_step_sounds), 50, TRUE, falloff_distance = 1, vary = sound_vary, ignore_walls = FALSE)
		else
			...
			if(!isnull(barefoot_type) && bare_footstep_sounds[barefoot_type]) // barefoot_type can be null
				heard_clients = playsound(source.loc, pick(bare_footstep_sounds[barefoot_type][1]),
					...
					TRUE,
					bare_footstep_sounds[barefoot_type][3] + e_range + range_adjustment, falloff_distance = 1, vary = sound_vary, ignore_walls = FALSE)
...
/datum/element/footstep/proc/play_simplestep_machine(atom/movable/source, atom/oldloc, direction, forced, list/old_locs, momentum_change)
	...
	playsound(source_loc, footstep_sounds, 50, falloff_distance = 1, vary = sound_vary, pressure_affected = TRUE, ignore_walls = FALSE)
